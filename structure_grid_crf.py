...
class GridCRFStructuredModel(DirectorStructuredModel):
	...
	def init_opt(self, A, a, B, b, lb, ub, C):
	...
	def _get_pairwise_potentials_(self, w):
		...
		'''
		# Get the last n_states elements of w
...
n_features = 2 # has to matched with the one used by toy.generate_*
# Number of possible values that each node may take
n_states = 2 # has to matched with the one used by toy.generate_*
...
sosvm = StochasticSubgradientSOSVM(model, loss, labels, debug_at_iteration=50)
sosvm.train()
...
if Plot:
	...
	for i in xrange(5):
		...
		pred_seq = Sequence.obtain_from_generic(predicted.get_label(i))
