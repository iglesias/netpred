...
#### binary
def generate_blocks(n_samples=10, noise=1.5, seed=0, n_rows=10, n_cols=12):
    np.random.seed(seed)
    Y = np.ones((n_samples, n_rows, n_cols))
    Y[:, :, :n_cols//2] = -1
...
def generate_checker(n_samples=10, noise=1.5, n_rows=11, n_cols=13):
    np.random.seed(0)
    Y = np.ones((n_samples, n_rows, n_cols))
    Y[:, ::2, ::2] = -1
    Y[:, 1::2, 1::2] = -1
...
#### Multinomial
def generate_blocks_multinomial(n_samples=20, noise=0.5, seed=None, n_rows=10, n_cols=12):
    if seed is not None:
        np.random.seed(seed)
